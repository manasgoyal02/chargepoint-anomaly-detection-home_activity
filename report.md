...
Isolation Forest uses an internal anomaly scoring mechanism based on how easily observations can be isolated. The contamination parameter was set to 0.02, meaning the model assumes approximately 2% of observations are anomalous.

This threshold was chosen to reflect the expectation that anomalies represent a small fraction of overall charging activity while still allowing detection of abnormal operational behavior.

Additionally, physics-based validation was used alongside model predictions to identify physically impossible or inconsistent charging behavior.

---

### Examples of Detected Anomalies

The model successfully identified several types of abnormal charging behavior, including:

- Observations with large deviations between expected power and actual power
- Sudden spikes or drops in power, voltage, or temperature
- Physically inconsistent values such as negative or abnormal electrical readings
- Charging sessions with unusual energy or duration patterns

These anomalies were confirmed through visual inspection and comparison with engineered physics-based features.

---

### Expected Behavior in Production

In a production environment, this model would continuously analyze incoming charging station telemetry data and assign anomaly labels in real time.

The model would be able to detect:

- Abnormal charging patterns
- Hardware inconsistencies
- Operational faults
- Physically inconsistent charging behavior

The inference pipeline developed in this project allows the trained model to be applied to new data without retraining.

Over time, the model could be retrained periodically using updated data to adapt to evolving charging patterns and improve detection performance.
